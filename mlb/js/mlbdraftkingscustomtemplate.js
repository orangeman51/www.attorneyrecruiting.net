$(function () {
    ...
    reader.onload = function(event){
      ...
      raw.forEach(function (player) {
        var b = {
          ...
          displaypos:getDisplayPosition(player.Pos.substring(player.Pos.indexOf("/")+1),player.Pos.substring(0,player.Pos.indexOf("/"))),
          handedness:getHandedness(player.Pos.substring(player.Pos.indexOf("/")+1),player.Matchup),
  ...
  function optimize(){
          ...
          if (PLAYERS[j].team == $('#schedule-away'+i).text()) {
            if (PLAYERS[j].handedness == "L") {
              if (PLAYERS[j].starting == "Starting" || PLAYERS[j].starting == "Projected Starting") {
              	if (containsObject(PLAYERS[j],allSelectedPlayers) !== true) {
                	allSelectedPlayers.push(PLAYERS[j]);
            	}
              }
            }
          }
    ...
    for (i = 0; i < PLIST.length; i++) {
      if (document.getElementById('checkbox-p'+i).checked) {
        if (containsObject(PLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(PLIST[i]);
        }
      }
    }
    for (i = 0; i < CLIST.length; i++) {
      if (document.getElementById('checkbox-c'+i).checked) {
        if (containsObject(CLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(CLIST[i]);
        }
      }
    }
    for (i = 0; i < FBLIST.length; i++) {
      if (document.getElementById('checkbox-fb'+i).checked) {
        if (containsObject(FBLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(FBLIST[i]);
        }
      }
    }
    for (i = 0; i < SBLIST.length; i++) {
      if (document.getElementById('checkbox-sb'+i).checked) {
        if (containsObject(SBLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(SBLIST[i]);
        }
      }
    }
    for (i = 0; i < TBLIST.length; i++) {
      if (document.getElementById('checkbox-tb'+i).checked) {
        if (containsObject(TBLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(TBLIST[i]);
        }
      }
    }
    for (i = 0; i < SSLIST.length; i++) {
      if (document.getElementById('checkbox-ss'+i).checked) {
        if (containsObject(SSLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(SSLIST[i]);
        }
      }
    }
    for (i = 0; i < OFLIST.length; i++) {
      if (document.getElementById('checkbox-of'+i).checked) {
        if (containsObject(OFLIST[i],allSelectedPlayers) !== true) {
          allSelectedPlayers.push(OFLIST[i]);
        }
      }
    }
    ...
    for (i = 0; i < playerCount; i++) {
      if (allSelectedPlayers[i].pos1 == "P" || allSelectedPlayers[i].pos2 == "P") {
        userPList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "C" || allSelectedPlayers[i].pos2 == "C") {
        userCList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "1B" || allSelectedPlayers[i].pos2 == "1B") {
        userFBList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "2B" || allSelectedPlayers[i].pos2 == "2B") {
        userSBList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "3B" || allSelectedPlayers[i].pos2 == "3B") {
        userTBList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "SS" || allSelectedPlayers[i].pos2 == "SS") {
        userSSList.push(allSelectedPlayers[i]);
      }
      if (allSelectedPlayers[i].pos1 == "OF" || allSelectedPlayers[i].pos2 == "OF") {
        userOFList.push(allSelectedPlayers[i]);
      }
    }
    ...
    if (pLength < 2) {
    	$("#missing-position-text").append("<strong>You must select at least two Pitchers.</strong><br>");
    }
    if (cLength < 1) {
    ...
    if (pLength < 2 || cLength < 1 || fbLength < 1 || sbLength < 1 || tbLength < 1 || ssLength < 1 || ofLength < 3) {
    ...
    if (STACKTEAM !== "none") {
      for (p1 = 0; p1 < pLength-1; p1++){
        LINEUP.p1 = userPList[p1];
        for (p2 = p1+1; p2 < pLength; p2++){
          LINEUP.p2 = userPList[p2];
          for (c = 0; c < cLength; c++){
  ...
  // INPUT 2 POSITIONS AND RETURN DISPLAY POSITION STRING
  function getDisplayPosition (pos1, pos2) {
    if (pos2.length > 0) {
      return pos1 + "/" + pos2;
    } else {
      return pos1;

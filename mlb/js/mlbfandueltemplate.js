$(function () {
  ...
  function optimize(){
    ...
    if (TEAMCOUNT <= 16) {
      ...
      var pitcherListByRatio = pitcherList.slice(0, 10);
      pitcherList.sort(sortByFP);
      var pitcherListByFP = pitcherList.slice(0, 6);
    ...
    for (i = 0; i < LOCKS.length; i++) {
	    if (LOCKS[i].position == "P") {
		    pitcherList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "C") {
		    catcherList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "1B") {
		    firstBaseList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "2B") {
		    secondBaseList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "3B") {
		    thirdBaseList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "SS") {
		    shortstopList = [LOCKS[i]];
		}
	    if (LOCKS[i].position == "OF") {
    ...
    for (i = 0; i < LINEUPSARRAY.length; i++) {
      ...
      $('#tbl-optimized' + i).append("<tr id='lineup-table-row-p" + i + "'><td id='lineup-team-p" + i + "'>" + LINEUPSARRAY[i].players.p.team + "</td><td>P</td><td id='inj-p" + i + "'>" + LINEUPSARRAY[i].players.p.inj + "</td><td id='lineup-name-p" + i + "'>" + LINEUPSARRAY[i].players.p.name + "</td><td>" + LINEUPSARRAY[i].players.p.fp.toFixed(2) + "</td><td>$" + commaSeparateNumber(LINEUPSARRAY[i].players.p.cost) + "</td><td>" + LINEUPSARRAY[i].players.p.percentOwned.toFixed(2) + "%</td><td class='lock-checkbox'><input id='lineup-lock-checkbox-p" + i + "'type='checkbox'></td><td class='exclude-checkbox'><input id='lineup-exclude-checkbox-p" + i + "'type='checkbox'></td></tr>");
  ...
  // SET EXCLUDE PARAMETER TO TRUE FOR ALL MEMBERS OF CHECKED TEAM
    ...
    if (LINEUPSARRAY.length > 0) {
      for (i = 0; i < LINEUPSARRAY.length; i++) {
        if (document.getElementById('lineup-exclude-checkbox-p'+i).checked) {
          for (j = PLAYERS.length - 1; j >= 0; j--){
            if (PLAYERS[j].name == $('#lineup-name-p'+i).text() && PLAYERS[j].team == $('#lineup-team-p'+i).text()) {
              PLAYERS[j].exclude = true;
            }
          }
        }
  ...
  function setLocks () {
    ...
    if (LINEUPSARRAY.length > 0) {
      for (i = 0; i < LINEUPSARRAY.length; i++) {
        if (document.getElementById('lineup-lock-checkbox-p'+i).checked) {
          for (j = PLAYERS.length - 1; j >= 0; j--){
            if (PLAYERS[j].name == $('#lineup-name-p'+i).text() && PLAYERS[j].team == $('#lineup-team-p'+i).text()) {
              PLAYERS[j].lock = true;
            }
  ...
  function clearLocks() {
    ...
    for (i = 0; i < LINEUPSARRAY.length; i++) {
      document.getElementById('lineup-lock-checkbox-p'+i).checked = false;
    }
    for (i = 0; i < LINEUPSARRAY.length; i++) {
  ...
  function clearExcludes() {
    ...
    for (i = 0; i < LINEUPSARRAY.length; i++) {
      document.getElementById('lineup-exclude-checkbox-p'+i).checked = false;
    }
    for (i = 0; i < LINEUPSARRAY.length; i++) {
  ...
  function incrementStackModeMap(team, modeMap) {  
    ...
    modeMap[team] = updatedCount;
    var countTeamStacks = 0;
    for (var k in modeMap) {
      if (modeMap[k] > 3 || (modeMap[k] > 2 && STACKOPTS.indexOf('ministack') > -1)) {
        countTeamStacks++;
      }
    }
    if(STACKOPTS.indexOf('doublestack') === -1) {  
      if(updatedCount > 3 || (updatedCount > 2 && STACKOPTS.indexOf('ministack') > -1)) {
        return false;
      }
    }
    else {
      if(countTeamStacks > 1) {
        return false;
      }      
    }
    return true;
  ...
  // CHECK IF LINEUP HAS MORE THAN 3 HITTERS FROM A TEAM AND RETURN TRUE OR FALSE
  ...
  function isSelectedStack (lineup, team) {
    ...
    for (var k = 0; k < countArr.length; k++) {
      if (countArr[k] > 3 || (countArr[k] > 2 && STACKOPTS.indexOf('ministack') > -1)) {
        countTeamStacks++;
      }
    }
    if(STACKOPTS.indexOf('doublestack') === -1) {
      if (countArr.indexOf(4) > -1 || (countArr.indexOf(3) > -1 && STACKOPTS.indexOf('ministack') > -1)) {
        return true;
      }
    }  
    else {
  ...
  function includesLocks(lineup, locks) {
  	if (locks.length == 0) {
	  	...
	  	var lineupArray = [lineup.p, lineup.c, lineup.fb, lineup.sb, lineup.tb, lineup.ss, lineup.of1, lineup.of2, lineup.of3];
	  	var count = 0;
	  	for (i = 0; i < locks.length; i++) {
